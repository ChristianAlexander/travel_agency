defmodule TravelAgency.AiAgentActorPersister do
  use AshOban.ActorPersister

  def store(%TravelAgency.Accounts.User{id: id}), do: %{"type" => "user", "id" => id}

  def lookup(%{"type" => "user", "id" => id}) do
    with {:ok, user} <- Ash.get(TravelAgency.Accounts.User, id, authorize?: false) do
