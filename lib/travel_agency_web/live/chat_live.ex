defmodule TravelAgencyWeb.ChatLive do
  use Elixir.TravelAgencyWeb, :live_view
  ...
  def mount(_params, _session, socket) do
    TravelAgencyWeb.Endpoint.subscribe("chat:conversations:#{socket.assigns.current_user.id}")
    ...
    socket =
      ...
      |> stream(
        :conversations,
        TravelAgency.Chat.my_conversations!(actor: socket.assigns.current_user)
      )
      |> assign(:messages, [])
  ...
  def handle_params(%{"conversation_id" => conversation_id}, _, socket) do
    conversation =
      TravelAgency.Chat.get_conversation!(conversation_id, actor: socket.assigns.current_user)
    ...
    cond do
      ...
      socket.assigns[:conversation] ->
        TravelAgencyWeb.Endpoint.unsubscribe("chat:messages:#{socket.assigns.conversation.id}")
        TravelAgencyWeb.Endpoint.subscribe("chat:messages:#{conversation.id}")
    ...
    |> stream(:messages, TravelAgency.Chat.message_history!(conversation.id, stream?: true))
    |> assign_message_form()
    |> then(&{:noreply, &1})
  end
  ...
  defp assign_message_form(socket) do
    form =
      if socket.assigns.conversation do
        TravelAgency.Chat.form_to_create_message(
          actor: socket.assigns.current_user,
          private_arguments: %{conversation_id: socket.assigns.conversation.id}
        )
        |> to_form()
      else
        TravelAgency.Chat.form_to_create_message(actor: socket.assigns.current_user)
        |> to_form()
      end
